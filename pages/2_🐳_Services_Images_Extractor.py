...
# 配置文件路径（均相对仓库根目录，避免工作目录不同导致写错文件）
PROJECT_ROOT = Path(__file__).resolve().parent.parent
ARGOCD_CONFIG_PATH = PROJECT_ROOT / "config" / "argocd_config.json"
...
# 加载配置
def load_config():
    """从 ``config/argocd_config.json`` 加载（含 ``service_mappings``）。"""
    try:
        if ARGOCD_CONFIG_PATH.is_file():
            with open(ARGOCD_CONFIG_PATH, 'r', encoding='utf-8') as f:
                config = json.load(f)
...
def save_config(config):
    """写入 ``config/argocd_config.json``（含 environment、services、service_mappings）。"""
    try:
        ARGOCD_CONFIG_PATH.parent.mkdir(parents=True, exist_ok=True)
        with open(ARGOCD_CONFIG_PATH, 'w', encoding='utf-8') as f:
...
def parseServiceMappings(text: str) -> dict[str, str]:
    """
    解析「别名=仓库目录名」多行文本。

    每行一条，格式为 ``别名=目录名``（仅分割首个 ``=``）。以 ``#`` 开头的行为注释。
    含 ``/`` 或 ``..`` 的键或值会被忽略，避免路径注入。
    ...
    for raw_line in text.splitlines():
        ...
        if not alias or not repo:
            continue
        if '/' in alias or '/' in repo or '..' in alias or '..' in repo:
            continue
        result[alias] = repo
    return result


def resolveRepoFolder(userLabel: str, mappings: dict[str, str]) -> str:
    """
    将用户侧服务名解析为仓库中的 overlay 目录名。

    Args:
        userLabel: 多选或自定义中的服务名
        mappings: ``parseServiceMappings`` 的返回值

    Returns:
        用于拼接 ``kustomize/overlays/<env>/<目录>/kustomization.yml`` 的目录名
    """
    return mappings.get(userLabel, userLabel)


def serviceMappingsToText(mappings: dict[str, str]) -> str:
    """
    将映射字典序列化为多行 ``别名=目录名``，键按字母序便于阅读。

    Args:
        mappings: 配置中的 service_mappings
...
def mergeServiceMappings(*parts: dict[str, str]) -> dict[str, str]:
    """
    合并多组映射，后传入的键覆盖先前的键。

    Args:
        *parts: 若干 ``别名 -> 仓库目录名`` 字典
...
def effectiveServiceMappings(
    argocdMappings: dict[str, str],
    textAreaRaw: str,
) -> dict[str, str]:
    """
    计算查询时使用的最终映射：以 ``argocd_config.json`` 的 ``service_mappings`` 为基础，
    与侧栏当前文本合并，**文本框中同名键覆盖**已保存项（便于未点保存时先试跑）。

    Args:
        argocdMappings: ``argocd_config['service_mappings']``
        textAreaRaw: 侧栏映射文本框内容

    Returns:
        合并后的映射
    """
    saved = argocdMappings or {}
    from_text = parseServiceMappings(textAreaRaw or '')
    return mergeServiceMappings(saved, from_text)
...
for _cfgKey in DEFAULT_CONFIG:
    if _cfgKey not in st.session_state.argocd_config:
        st.session_state.argocd_config[_cfgKey] = DEFAULT_CONFIG[_cfgKey]
...
if 'last_updates' not in st.session_state:
    st.session_state.last_updates = None
...
# 反向查询相关状态
if 'reverse_query_results' not in st.session_state:
    st.session_state.reverse_query_results = None
if 'reverse_last_updates' not in st.session_state:
    st.session_state.reverse_last_updates = None
if 'reverse_query_time' not in st.session_state:
    st.session_state.reverse_query_time = None
...
# 侧边栏配置
with st.sidebar:
    # 必须在带 key 的 text_area 之前执行：重置时删除 widget 状态，避免
    # ``st.session_state.service_mappings_text = ...`` 触发 StreamlitAPIException
    if st.session_state.pop('_argocd_reset_pending', False):
        st.session_state.argocd_config = DEFAULT_CONFIG.copy()
        save_config(DEFAULT_CONFIG.copy())
        st.session_state.pop('service_mappings_text', None)
        st.success("🔄 配置已重置")
    ...
    st.markdown("**服务名映射：**")
    st.caption(
        "与 ``argocd_config.json`` 中 ``service_mappings`` 一致；每行 ``别名=仓库目录名``。"
        " 点击「💾 保存」写入该文件。未保存时先点「开始提取」会以当前文本覆盖同名键试跑。"
    )
    if 'service_mappings_text' not in st.session_state:
        _sm = st.session_state.argocd_config.get('service_mappings', {}) or {}
        st.session_state.service_mappings_text = serviceMappingsToText(_sm)
    ...
    with col1:
        if st.button("💾 保存", use_container_width=True):
            _mappings = parseServiceMappings(
                st.session_state.get('service_mappings_text', '')
            )
            config = {
                'environment': environment,
                'services': services_list,
                'service_mappings': _mappings
            }
    ...
    with col2:
        if st.button("🔄 重置", use_container_width=True, key="reset_argocd_config_btn"):
            st.session_state._argocd_reset_pending = True
            st.rerun()
...
# 计算反向查询的剩余服务列表
_remaining_services = calculateRemainingServices(available_services, services_list)
...
with col1:
    ...
    if st.button("🗑️ 清空历史结果", use_container_width=True, help="清空 results 目录文件和浏览器对比数据"):
        ...
        # 2. 清空浏览器 session_state 中的对比数据（包括反向查询结果）
        st.session_state.query_results = None
        st.session_state.previous_results = None
        st.session_state.comparison_data = None
        st.session_state.last_query_time = None
...
with col2:
    query_button = st.button(
        ...
        help=f"查询已选择的 {len(services_list)} 个发版服务"
    )

with col3:
    reverse_query_button = st.button(
        "🔄 反向查询（剩余服务）",
        type="secondary",
        use_container_width=True,
        disabled=not _remaining_services,
        help=f"查询剩余 {len(_remaining_services)} 个非发版服务"
    )
    if _remaining_services:
        st.caption(f"剩余 {len(_remaining_services)} 个服务")
...
if query_button:
    if not services_list:
        ...
        try:
            ...
            # 并发查询：内置 JSON + 已保存 + 文本框合并后解析路径，结果键仍为用户侧名称
            mappings = effectiveServiceMappings(
                st.session_state.argocd_config.get('service_mappings', {}) or {},
                st.session_state.get('service_mappings_text', ''),
            )
            repo_list = list(
                dict.fromkeys(
                    resolveRepoFolder(s, mappings) for s in services_list
                )
            )
            results = {
                'success': {},
            ...
            with st.spinner("🚀 正在并发查询所有服务..."):
                batch_results = client.query_multiple_services(repo_list)
            batch_success = batch_results.get('success', {})
            batch_failed = batch_results.get('failed', {})
            batch_last_updates = batch_results.get('last_updates', {})

            for user_label in services_list:
                repo = resolveRepoFolder(user_label, mappings)
                if repo in batch_success:
                    results['success'][user_label] = batch_success[repo]
                elif repo in batch_failed:
                    results['failed'][user_label] = batch_failed[repo]

            display_name = client.env_config['display_name']
            for user_label, err in results['failed'].items():
                if '文件不存在' in err or '404' in err:
                    repo = resolveRepoFolder(user_label, mappings)
                    suffix = (
                        f"（仓库目录名: {repo}）" if repo != user_label else ''
                    )
            ...
            # 按 services_list 顺序重建 details（供表格渲染用）
            ...
            # 同时保存 last_updates 到 session_state（用于对比时显示）
            st.session_state.last_updates = batch_last_updates
...
# 执行反向查询
if reverse_query_button:
    if not _remaining_services:
        st.error("❌ 没有剩余服务可查询")
    else:
        try:
            client = GitHubKustomizeClient(current_environment, token)

            st.subheader(f"🔄 反向查询 {current_environment.upper()} 环境（剩余服务）")
            ...
            repo_list = list(
                dict.fromkeys(
                    resolveRepoFolder(s, mappings) for s in _remaining_services
                )
            )
            results = {
                'success': {},
            ...
            with st.spinner(f"🔄 正在并发查询 {len(_remaining_services)} 个剩余服务..."):
                batch_results = client.query_multiple_services(repo_list)
            batch_success = batch_results.get('success', {})
            batch_failed = batch_results.get('failed', {})
            batch_last_updates = batch_results.get('last_updates', {})

            for user_label in _remaining_services:
                repo = resolveRepoFolder(user_label, mappings)
                if repo in batch_success:
                    results['success'][user_label] = batch_success[repo]
                elif repo in batch_failed:
            ...
            st.session_state.reverse_query_results = results
...
# 显示反向查询结果（使用 expander 折叠展示）
if st.session_state.reverse_query_results:
    reverse_results = st.session_state.reverse_query_results
    remaining_count = len(_remaining_services)

    with st.expander(
        f"🔄 反向查询结果（剩余服务 {remaining_count} 个）",
        expanded=False
    ):
        # 统计信息
        col1, col2, col3, col4 = st.columns(4)
        ...
        with col1:
            st.metric("🎯 查询服务数", remaining_count)

        with col2:
            st.metric("✅ 成功", len(reverse_results['success']))
        ...
        **分支状态说明:**
        ...
        # 结果表格
        if reverse_results['details']:
            st.markdown("#### 📋 剩余服务详细结果")
            df_reverse = pd.DataFrame(reverse_results['details'])
        ...
        # 失败详情
        if reverse_results['failed']:
            st.markdown("---")
            st.markdown("#### ⚠️ 失败详情")
            for service, error in reverse_results['failed'].items():
                st.error(f"**{service}**: {error}")

        # 查询时间
        if st.session_state.reverse_query_time:
            st.caption(f"查询时间: {st.session_state.reverse_query_time.strftime('%Y-%m-%d %H:%M:%S')}")
