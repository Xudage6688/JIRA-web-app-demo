...
FALLBACK_CONFIG = {
  ...
  "sp_team_field": "customfield_10002",
  "resolutions": [
...
class JiraOperationsClient:
  ...
  def __init__(self, base_url: str, email: str, api_token: str):
    ...
    Args:
      base_url: Jira 实例 URL (e.g., https://your-company.atlassian.net)
      email: 用户邮箱
  ...
  def get_create_metadata(self, project_key: str = "DEMO") -> Optional[Dict[str, Any]]:
    """
    获取创建 Issue 的元数据（issue types, priorities, custom fields）

    Args:
      project_key: 项目 key，默认 "DEMO"
    ...
    try:
      ...
      # 查找 SP Team 字段（优先使用 customfield_10002）
      sp_team_candidates = ['customfield_10002']  # 已知的 SP Team 字段
  ...
  def create_issue(
    ...
    Args:
      ...
      bug_category: Bug 分类（customfield_10004）
      sprint_id: Sprint ID
    ...
    if environment_occured:
      fields["customfield_10003"] = [{"value": environment_occured}]

    # 添加 Bug Category（customfield_10004）
    if bug_category:
      fields["customfield_10004"] = {"value": bug_category}
    ...
    if response and response.get('success') and sprint_id:
      ...
      if issue_id:
        logger.debug(f"创建 Issue 成功: {issue_key}，正在添加到 Sprint {sprint_id}")
        ...
        if sprint_response and sprint_response.get('success'):
          logger.debug(f"成功将 {issue_key} 添加到 Sprint {sprint_id}")
        else:
          logger.warning(f"添加到 Sprint 失败，但 Issue 创建成功: {issue_key}")
          # 不影响创建成功的返回
  ...
  def get_issue(self, issue_key: str) -> Dict[str, Any]:
    ...
    Args:
      issue_key: Issue key (e.g., DEMO-10001)
  ...
  def get_active_sprints(
    ...
    """
    try:
      logger.debug("get_active_sprints: 使用 Agile API 获取 Active Sprints")
      ...
      if not (boards_response and boards_response['success']):
          logger.debug("无法获取 Board 列表")
          return []

      boards = boards_response['data'].get('values', [])
      logger.debug(f"找到 {len(boards)} 个 Scrum Board")
      ...
      if team_name and team_name != "（不设置）":
          ...
          logger.debug(f"Team '{team_name}' 过滤后剩余 {len(boards)} 个 Board")

      # 指定了 board_ids 时进一步限制
      if board_ids is not None:
          boards = [b for b in boards if b.get('id') in board_ids]
          logger.debug(f"board_ids 过滤后剩余 {len(boards)} 个 Board")
      ...
      with ThreadPoolExecutor(max_workers=10) as executor:
          ...
          for future in as_completed(futures):
              try:
                  ...
                  logger.warning(f"Board '{board_name}' 查询异常: {exc}")

      logger.debug(f"总共找到 {len(all_sprints)} 个 Active Sprint")
      return list(all_sprints.values())

    except Exception as e:
      logger.error(f"获取 Sprint 失败: {str(e)}")
      logger.exception("操作失败详情")
      return []
  ...
  def get_sprints_by_team(self, team_name: Optional[str] = None) -> List[Dict[str, Any]]:
    ...
    Args:
      team_name: Team 名称（如 "Team-Epsilon"），None 则返回全部
    """
    try:
      sprints = self.get_active_sprints(team_name=team_name)
      logger.debug(f"get_sprints_by_team: 获取到 {len(sprints)} 个总 Sprint")
      return sprints
    except Exception as e:
      logger.error(f"get_sprints_by_team 失败: {str(e)}")
      logger.exception("操作失败详情")
      return []
