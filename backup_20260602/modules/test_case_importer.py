...
def render_test_case_importer_tab(
    ...
    with tab_upload:
        ...
        uploaded_file = st.file_uploader(
            ...
            type=["xlsx", "csv"],
            help="支持 Excel 和 CSV 格式，确保包含 Action / Data / Expected Result 三列",
    ...
    # ---------- Tab 3: 从剪切板粘贴 ----------
    with tab_clipboard:
        st.markdown("直接粘贴 CSV 格式文本（以 Tab 分隔），系统将自动解析并加载内容。")
        st.markdown("**格式要求**：第一行为列头（Action / Data / Expected Result），后续每行为一条 Test Case。")

        clipboard_text = st.text_area(
            "粘贴 CSV 文本",
            placeholder="Action\tData\tExpected Result\nCustomer setting a customize rate then booking a LT order\tUSE:EUR=1.5\tDisplay new customize rate in LT program page",
            height=250,
            key="clipboard_input"
        )
        ...
        if parse_btn and clipboard_text:
            try:
                ...
                if len(lines) < 2:
                    ...
                    for h in headers:
                        ...
                        if h_lower == "action":
                            ...
                            col_map[h] = "Data"
                        elif "expected" in h_lower or "result" in h_lower:
                            col_map[h] = "Expected Result"
                    ...
                    if "Action" not in mapped_headers or "Expected Result" not in mapped_headers:
                        ...
                        if not data:
                            ...
                            st.dataframe(preview_df_clip, use_container_width=True)

                            st.session_state.clipboard_parsed_df = df_clip
                            df_cases = df_clip
        ...
        if df_cases is None and st.session_state.get("clipboard_parsed_df") is not None:
            df_cases = st.session_state.clipboard_parsed_df
            ...
            try:
                ...
                if created_keys and related_ticket_clean:
                    ...
                    if is_test_set and numeric_id:
                        ...
                        if test_numeric_ids:
                            try:
                                link_successes, link_failures, xray_warning = link_tests_to_test_set(
