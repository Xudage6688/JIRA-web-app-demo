...
# SSL 验证配置：默认启用，可通过 ARGOCD_SSL_VERIFY=False 环境变量禁用
def _get_ssl_verify() -> bool:
    """动态读取 SSL 验证配置"""
    return os.environ.get('ARGOCD_SSL_VERIFY', 'True').lower() != 'false'
...
class ArgoCDClient:
    ...
    def get_application(self, app_name: str) -> Optional[Dict]:
        ...
        try:
            response = requests.get(url, headers=self.headers, verify=_get_ssl_verify(), timeout=30)
    ...
    @staticmethod
    @st.cache_data(ttl=3600)  # 缓存1小时，环境配置基本不变
    ...
    @staticmethod
    @st.cache_data(ttl=3600)  # 缓存1小时，环境列表很少变化
